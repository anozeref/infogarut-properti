...
const TabelUserAktif = ({ users, properties, actions, theme }) => {
              ...
              return (
                <motion.tr key={user.id} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
                  ...
                  <td className={styles.statusCell}>
                    {user.verified?<span className={styles.approved}>✔ Terverifikasi</span>:<span className={styles.pending}>⌛ Belum</span>}
                  </td>
                  <td>{user.joinedAt}</td>
