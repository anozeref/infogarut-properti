...
const TabelUserBanned = ({ users, properties, actions }) => {
              ...
              {paginate(users).map((user,idx)=>{
                ...
                return (
                  <motion.tr key={user.id} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
                    ...
                    <td className={styles.statusCell}><span className={styles.bannedBadge}>❌ Banned</span></td>
                    <td>{user.joinedAt}</td>
...
export default TabelUserBanned;
